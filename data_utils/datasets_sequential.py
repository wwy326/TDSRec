...
class SequentialDataset(data.Dataset):
    def __init__(self, user_seqs, mode='train', user_seqs_aug=None):
        ...
        if user_seqs_aug is not None:
            ...
            self.seqs_time = user_seqs_aug["item_time_seq"]
            self.last_items_time = user_seqs_aug["item_time_id"]
        else:
            self.uids = user_seqs["uid"]
            self.seqs = user_seqs["item_seq"]
            self.last_items = user_seqs["item_id"]
            self.seqs_time = user_seqs["item_time_seq"]
            self.last_items_time = user_seqs["item_time_id"]

        if mode == 'test':
            self.test_users = self.uids
            self.user_pos_lists = np.asarray(
                self.last_items, dtype=np.int32).reshape(-1, 1).tolist()
            self.user_pos_time_lists = np.asarray(
                self.last_items_time, dtype=np.int32).reshape(-1, 1).tolist()
    ...
    def __getitem__(self, idx):
        seq_i = self.seqs[idx]
        seq_i_time = self.seqs_time[idx]
        if self.mode == 'train' and 'neg_samp' in configs['data'] and configs['data']['neg_samp']:
            return self.uids[idx], torch.LongTensor(self._pad_seq(seq_i)), torch.LongTensor(self._pad_seq(seq_i_time)), self.last_items_time[idx], self.last_items[idx], self.negs[idx],
        else:
            return self.uids[idx], torch.LongTensor(self._pad_seq(seq_i)), self.last_items[idx], torch.FloatTensor(self._pad_seq(seq_i_time)), self.last_items_time[idx],
